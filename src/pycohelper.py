...
def percSplitOuter(df, scanNrTest, peptideTest):
    if (scanNrTest):
...
def percSelectTrain(training, qTrain, rankOption, lowRankDecoy):
    if(rankOption):
...
def percInitClf(falseTrain, trueTrain, train, classes, balancedOption, KFoldTest, scanNrTest, peptideTest):
